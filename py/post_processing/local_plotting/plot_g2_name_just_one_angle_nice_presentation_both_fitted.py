...
axs.plot(at25_25[0], at25_25[1], "r--" ,  linewidth=3 ,label = f"Simulation: Interaction on")   
axs.plot(-at25_25[0], at25_25[1], "r--" , linewidth=3 )   
...
try:
    ...
    plt.plot(taulist, func(taulist, *popt), 'r-', linewidth=3, label=r'Empirical fit with interaction:' + "\n"+ ' $\Delta$=%5.2f, f=%5.2f, $\chi$=%5.2f' % tuple(popt))
except:
    print("ugly fit")
    pass
...
axs.set_title(r"$g^{(2)}(\tau)$ "+ "oppposite directions with" + f" $b_0 = {b0_input}, \Omega = {Omega}, \Delta $ = {Delta}  ")
axs.plot(at25_25[0], at25_25[1], "b--", linewidth=3 , label = f"Simulation: Interaction off")   
axs.plot(-at25_25[0], at25_25[1],"b--", linewidth=3  )   

try:
    func = second_order_correlation_opposite_directions_interaction_on_empirical_araujo
    popt, pcov = curve_fit(func, at25_25[0], at25_25[1]) 
    plt.plot(taulist, func(taulist, *popt), 'b-', linewidth=3, label=r'Empirical fit with interaction:' + "\n"+ ' $\Delta$=%5.2f, f=%5.2f, $\chi$=%5.2f' % tuple(popt))
except:
    print("ugly fit")
    pass


axs.set_xlabel(r"$\tau$", fontsize=18)
axs.set_ylabel(r"$g^{(2)}(\tau)$")
#Curva Teo
g12 = second_order_correlation_opposite_directions_interaction_off_araujo(taulist, float(Delta))
axs.plot(taulist, g12, label = "Theory: Single atom approach")
...
plt.savefig(general_name + "nice_picture_both_fitted" + f"25_{angle}_manual_with_fit_comparison_mean.png", dpi = 300, bbox_inches="tight")
